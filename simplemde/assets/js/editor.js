(function ($) {
    ...
    $.fn.simplemde = function () {
        ...
        return this.each(function () {
            ...
            var fileUrl = simplemde.data("json") + '/file.json';
            ...
            $.ajax({
                ...
                success: function (translation, simplemde = simplemde) {
                    ...
                    // Filelink
                    $(".field-with-simplemde").find(".editor-toolbar").data("filelink-placeholder", translation["filelink.placeholder"] + "...");
                }
            });
            ...
            var toolbarItems = [{
                ...
                },
                {
                    name: "divider",
                    action: '',
                    className: "fa-header simplemde-divider",
                ...
                {
                    name: "filelink",
                    action: function filelinkFunction() {
                        ...
                        if (field.find(".pagesearch").length) {
                            ...
                            var input = $('<input type="text" class="pagesearch" placeholder="' + field.find(".editor-toolbar").data("filelink-placeholder") + '">');
                            field.find(".editor-toolbar").append(input);
                        }
                        ...
                        var index = {
                            url: function (phrase) {
                                return fileUrl + "?phrase=" + phrase + "&page=" + currentPage;
                            },
                            getValue: "safeName",
                            template: {
                                type: "custom",
                                method: function (value, item) {
                                    return '<span class="title">' + value + '</span>' + '<span class="uri"> (' + item.url + ')</span>';
                                }
                            },
                            list: {
                                maxNumberOfElements: 100,
                                onChooseEvent: function () {
                                    ...
                                    if (!selection) {
                                        if(fileType == 'image') {
                                            var replacement = '(image: ' + title + ')';
                                        } else {
                                            var replacement = '(file: ' + title + ' text: )';
                                        }
                                    } else {
                                        var replacement = '(file: ' + title + ' text: ' + selection + ')';
                                    }
                ...
                {
                    name: "tel",
                    action: function telFunction() {
                        var cm = simplemde.codemirror;
                        var selection = cm.getSelection();
                        var regex = /([0-9\s\-]{7,})(?:\s*(?:#|x\.?|ext\.?|extension)\s*(\d+))?$/g;
                        var text = '';
                        var tel = '';
                        ...
                        if (selection) {
                            ...
                            var replacement = '(tel: ' + tel + ' text: ' + text + ')';
                        } else {
                            var replacement = '(tel: )';
                        }
